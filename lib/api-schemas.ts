 ...
 * Uso em API routes:
 * ```ts
 * import { VendaRequestSchema } from '@/lib/api-schemas'
 * 
...
/**
 * Schema para POST /api/checkout/lead
 */
export const LeadRequestSchema = z.object({
  nome: z.string().min(3, 'Nome deve ter no mínimo 3 caracteres'),
  email: z.string().email('Email inválido'),
  telefone: z.string().regex(/^\(?\d{2}\)?\s?\d{4,5}-?\d{4}$/, 'Telefone inválido'),
  unidade: UnitSlugSchema.optional(),
  interesse: z.enum(['plano', 'aula_experimental', 'day_use', 'informacoes']).optional(),
