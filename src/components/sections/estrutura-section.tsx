...
import { useStructureFeaturesData, useEstruturaSectionData } from "../../../hooks/use-sanity-data"
...
export default function EstruturaSection() {
  const { data: structureData, loading: structureLoading } = useStructureFeaturesData()
  const { data: sectionData, loading: sectionLoading } = useEstruturaSectionData()

  const loading = structureLoading || sectionLoading
  ...
  const { header, additionalInfo, displaySettings } = sectionData
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {structureData.map((item, index) => (
            ...
            >
              {/* Imagem do Card */}
              <div className="relative h-48 bg-gradient-to-br from-yellow-400/20 to-amber-500/10 overflow-hidden">
                {item.image ? (
                  <>
                    <Image
        ...
        {displaySettings?.showAdditionalInfo && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
